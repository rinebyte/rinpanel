...
export const ConfigEditorDialog = forwardRef<ConfigEditorDialogHandle, Props>(function ConfigEditorDialog({ id, domain }, ref) {
  ...
  const handleReset = () => {
    if (!confirm("Mengembalikan konfigurasi ke pengaturan standar. Perubahan kustom akan hilang. Lanjutkan?")) return;
    ...
    >
      <form action={formAction} className="flex flex-col gap-4 p-6">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="eyebrow">konfigurasi · sunting</p>
          ...
          >
            ...
            {resetPending ? "[ ·· ] mengembalikan" : "Kembalikan ke Standar"}
          </button>
        </div>

        <p className="font-mono text-[0.7rem] text-zinc-500">
          Konfigurasi akan diperiksa terlebih dahulu. Jika terdapat kesalahan format, perubahan akan ditolak dan konfigurasi lama tetap berjalan tanpa gangguan.
        </p>

        {loadError && (
          <p className="rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 font-mono text-xs text-red-300">Gagal memuat konfigurasi: {loadError}</p>
        )}

        {resetError && (
          <p className="rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 font-mono text-xs text-red-300">Gagal mengembalikan konfigurasi: {resetError}</p>
        )}
        ...
        <textarea
          ...
          placeholder={!loaded ? "[ ·· ] memuat konfigurasi…" : ""}
        ...
        {state && !state.ok && state.error && (
          <div className="rounded-md border border-red-500/30 bg-red-500/10 p-3">
            <p className="flex items-center gap-2 font-mono text-xs text-red-300">
              ...
              Konfigurasi ditolak karena terdapat kesalahan:
            </p>
